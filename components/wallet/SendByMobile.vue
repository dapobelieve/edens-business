<template>
  <div class="modal-content">
    <div class="modal-header px-4">
      <h5 class="modal-title">Send Money (Mobile Money)</h5>
    ...
    <div class="modal-body">
      <div class="px-2" style="height: 23rem">
        <div class="mb-6">
        ...
        <div class="mb-4">
          <label class="eden-text-input" :class="$v.form.mobile_number.$error ? 'eden-text-input-error': ''">
            <input v-model="$v.form.mobile_number.$model" class="form-control" placeholder="Mobile Money Number">
          </label>
          <small class="fs-8 text-bad-red" v-if="$v.form.mobile_number.$error">{{accErrs}}</small>
        </div>
        <div class="mb-6">
...
export default {
  ...
  computed:  {
    ...
    accErrs() {
      if (!this.$v.form.mobile_number.required) return "Account Number required"
      if (!this.$v.form.mobile_number.numeric) return "Only numbers allowed"
    },
  },
  methods: {
    next() {
